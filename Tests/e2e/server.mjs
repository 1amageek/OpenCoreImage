...
// Serves Examples/SmokeTest/web/* plus the freshly built OCISmoke.wasm.
//
// build.sh lays the artifact out as:
//   Examples/SmokeTest/web/         -> index.html, app.js, runtime.mjs (checked in)
//   Examples/SmokeTest/web/OCISmoke.wasm (produced by build.sh, gitignored)
// This server mounts that directory root-level so `fetch('OCISmoke.wasm')`
...
async function preflight() {
    const wasmFile = join(WEB_ROOT, "OCISmoke.wasm");
    try {
        await stat(wasmFile);
    } catch {
        console.error(
            `\n✗ OCISmoke.wasm missing at ${wasmFile}\n` +
            `  Build it first:\n` +
            `    ./build.sh\n` +
...
createServer(serve).listen(PORT, "127.0.0.1", () => {
    console.log(`OpenCoreImage E2E server listening on http://127.0.0.1:${PORT}`);
